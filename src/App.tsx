...
export default function App() {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem("srl.theme") ?? "");
  const dark = theme ? theme === "dark" : matchMedia("(prefers-color-scheme: dark)").matches;
  const flip = () => {
    const t = dark ? "light" : "dark";
    localStorage.setItem("srl.theme", t);
  ...
  return (
    <div className="screen">
      <div className="top">
        ...
        <h1>{TITLES[view]}</h1>
        <button className="iconbtn" onClick={flip} aria-label={dark ? "Switch to light mode" : "Switch to dark mode"}>
          <Icon d={dark ? icons.sun : icons.moon} size={19} />
        </button>
      </div>
      <div key={epoch}>
